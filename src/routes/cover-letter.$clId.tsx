...
export const Route = createFileRoute("/cover-letter/$clId")({
	loader: async ({ params }) => {
		...
		try {
			...
			const cl = await client.query(api.coverLetters.getPublic, {
				id: params.clId as any,
			});
			if (!cl) {
				return { meta: null };
			}
			return {
				meta: {
					...
					ownerName: (cl.contactInfo?.fullName as string) ?? "",
				},
			};
		} catch {
			return { meta: null };
	...
	head: ({ loaderData }) => {
		...
		if (!meta) {
			return {
				meta: [
					{ title: "Shared Cover Letter — Alto CV" },
		...
		const description = meta.ownerName
			? `${meta.ownerName}'s cover letter, built with Alto CV.`
			: `A cover letter built with Alto CV.`;
		return {
			meta: [
				{ title: pageTitle },
